import { r as registerTemplate, p as parseSVGFragment, f as freezeTemplate, a as registerComponent, _ as _tmpl } from './lightning-button-wc.js';

const $fragment1$E = parseSVGFragment`<g${3}><path fill="#fcc003" d="M508 1A507 507 0 000 508v5384c0 280 227 508 508 508h4577c280 0 507-228 507-508V2032L3702 1H507z"${3}/><path fill="#e4a201" d="M5592 2036v100H4312s-631-126-612-671c0 0 25 571 600 571z"${3}/><path fill="#f9e3b6" d="M3702 1v1456c0 165 110 579 610 579h1280z"${3}/><path fill="#2e2204" d="M2014 5392c-31 0-60-18-72-48l-90-229h-598l-90 229a78 78 0 01-72 48 80 80 0 01-76-108l414-1033a130 130 0 01241 0l418 1033c4 9 6 20 6 29 0 36-33 79-81 79zm-461-1057l-256 647h509zm851 1057c-42 0-74-32-74-74V4242c0-40 32-72 77-72 40 0 72 32 72 72v1076c0 42-32 74-75 74z"${3}/></g>`;
function tmpl$E($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$E(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_ai = registerTemplate(tmpl$E);
tmpl$E.stylesheets = [];
tmpl$E.stylesheetToken = "lwc-5s717glko6r";
tmpl$E.legacyStylesheetToken = "buildTemplates-doctype_ai";
freezeTemplate(tmpl$E);

const $fragment1$D = parseSVGFragment`<g${3}><path fill="#747474" d="M511-3A507 507 0 004 505v5384a507 507 0 00507 507h4578a507 507 0 00507-507V2028L3706-3z"${3}/><path fill="#5c5c5c" d="M5598 2035v100H4318s-631-126-613-671c0 0 21 571 600 571z"${3}/><path fill="#c9c9c9" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/><path fill="#fff" d="M2660 4146c113-113 113-299 0-412s-297-113-412 0l-917 916c-113 113-113 299 0 412s299 113 412 0l562-560c32-32 32-85 0-118s-86-32-118 0l-353 353a120 120 0 01-176 0 120 120 0 010-176l351-354c132-130 343-130 472 0 130 132 130 343 0 472l-563 563c-211 216-553 211-766 0-211-213-211-556 0-766l917-917c213-213 556-213 766 0a539 539 0 010 766l-90 90a497 497 0 00-93-261l7-7z"${3}/></g>`;
function tmpl$D($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$D(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_attachment = registerTemplate(tmpl$D);
tmpl$D.stylesheets = [];
tmpl$D.stylesheetToken = "lwc-7o5urbs4pdk";
tmpl$D.legacyStylesheetToken = "buildTemplates-doctype_attachment";
freezeTemplate(tmpl$D);

const $fragment1$C = parseSVGFragment`<g${3}><path fill="#0d9dda" d="M515 1A507 507 0 008 509v5384c0 280 227 507 507 507h4578c280 0 507-227 507-507V2032L3710 1z"${3}/><path fill="#05628a" d="M5600 2036v100H4320s-631-126-613-671c0 0 21 571 600 571z"${3}/><path fill="#90d0fe" d="M3710 1v1456c0 165 110 579 610 579h1280z"${3}/><path fill="#fff" d="M2980 3404l-1417 181v1344c-74-21-163-24-253-7-201 40-332 171-294 292 39 122 233 188 434 148 176-35 299-139 300-247V4041l1043-129v831c-75-21-166-25-258-7-203 40-335 172-296 295 38 123 235 189 437 150 185-37 312-150 304-263z"${3}/></g>`;
function tmpl$C($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$C(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_audio = registerTemplate(tmpl$C);
tmpl$C.stylesheets = [];
tmpl$C.stylesheetToken = "lwc-4k053n2f9k";
tmpl$C.legacyStylesheetToken = "buildTemplates-doctype_audio";
freezeTemplate(tmpl$C);

const $fragment1$B = parseSVGFragment`<g${3}><path fill="#056764" d="M511 1A507 507 0 004 509v5384c0 280 227 507 507 507h4578c280 0 507-227 507-507V2032L3706 1z"${3}/><path fill="#024d4c" d="M5596 2038v100H4316s-631-126-613-671c0 0 21 571 601 571z"${3}/><path fill="#acf3e4" d="M3706 3v1456c0 165 110 579 610 579h1280z"${3}/><path fill="#fff" d="M2709 3933l211-210H1050s-37 2-37 40v135c0 19 18 35 38 35zm-732 727l210-210H1050s-37 1-37 40v135c0 19 19 35 38 35zm-506 730l27-210h-448s-37 2-37 40v135c0 19 19 35 38 35zm88-3s167-3 201-11 41-14 53-27l1311-1310s-50 42-159-69c-87-87-49-140-49-140L1617 5129c-24 26-25 40-27 51-3 10-31 207-31 207zm1515-1716l-106 106c-9 12-18 23-10 58s68 136 166 163c0 0 29 11 54-13l109-110s20-17 4-62c-16-44-81-124-158-154-39-14-59 12-59 12z"${3}/></g>`;
function tmpl$B($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$B(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_box_notes = registerTemplate(tmpl$B);
tmpl$B.stylesheets = [];
tmpl$B.stylesheetToken = "lwc-2qbvdbpujlt";
tmpl$B.legacyStylesheetToken = "buildTemplates-doctype_box_notes";
freezeTemplate(tmpl$B);

const $fragment1$A = parseSVGFragment`<g${3}><path fill="#2e844a" d="M511 0A507 507 0 003 507v5385c0 280 227 507 508 507h4577c280 0 507-227 507-508V2032L3705 0z"${3}/><path fill="#fff" d="M2031 4320c12 14 19 32 19 52 0 38-30 72-70 72-18 0-38-7-50-23a406 406 0 00-301-139c-263 0-463 202-463 488 0 284 200 488 463 488 113 0 225-49 301-137 12-15 32-23 50-23 42 0 70 35 70 73 0 18-7 36-19 51a535 535 0 01-402 169c-346 0-617-252-617-621s271-621 617-621c180 0 310 72 402 171zm680 1071c-178 0-319-59-421-149a71 71 0 01-24-54c0-36 27-76 70-76 15 0 31 4 43 14 83 74 198 132 337 132 214 0 283-116 283-207 0-310-711-139-711-568 0-198 176-333 412-333 155 0 288 47 385 128 16 14 25 34 25 54 0 36-30 72-70 72-14 0-30-5-43-16a483 483 0 00-308-105c-144 1-247 78-247 191 0 270 712 116 712 564 0 175-119 353-443 353zm1699-1125l-416 1033c-19 49-68 81-118 81h-4c-52 0-101-32-121-81l-414-1033c-4-9-5-18-5-29 0-36 32-79 81-79 30 0 59 18 72 48l389 999 388-999c11-29 40-48 72-48a80 80 0 0176 108z"${3}/><path fill="#194e31" d="M5600 2036v100H4320s-631-126-613-671c0 0 21 571 601 571z"${3}/><path fill="#cdefc4" d="M3710 1v1456c0 165 110 579 610 579h1280z"${3}/></g>`;
function tmpl$A($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$A(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_csv = registerTemplate(tmpl$A);
tmpl$A.stylesheets = [];
tmpl$A.stylesheetToken = "lwc-37q2k3gtl3f";
tmpl$A.legacyStylesheetToken = "buildTemplates-doctype_csv";
freezeTemplate(tmpl$A);

const $fragment1$z = parseSVGFragment`<g${3}><path fill="#fcc003" d="M511 1A507 507 0 003 508v5384c0 281 227 508 508 508h4577c280 0 507-227 507-508V2032L3705 1z"${3}/><path fill="#2e2204" d="M1745 5391h-625c-59 0-108-49-108-108v-985c0-59 49-108 108-108h625c36 0 65 29 65 69 0 35-29 64-65 64h-583v387h570c37 0 65 29 65 69 0 36-29 65-65 65h-570v414h583c36 0 65 29 65 65 0 39-29 68-65 68zm819-477h-333v414c0 42-33 74-76 74-41 0-74-32-74-74V4298c0-59 49-108 109-108h374c241 0 376 166 376 362s-138 362-376 362zm-18-591h-315v458h315c142 0 239-94 239-229s-97-229-239-229zm1026 1090c-178 0-319-60-421-150a71 71 0 01-24-54c0-36 27-76 70-76 15 1 31 4 43 15 83 74 199 131 337 131 214 0 283-115 283-207 0-310-711-138-711-567 0-198 176-333 412-333 155 0 288 47 385 128 16 14 25 34 25 54 0 36-30 72-70 72-14 0-30-6-43-16a483 483 0 00-308-105c-144 0-247 78-247 191 0 270 712 115 712 564 0 174-119 353-443 353z"${3}/><path fill="#e4a201" d="M5600 2036v100H4320c-244 0-631-126-613-671 0 0 21 571 601 571z"${3}/><path fill="#f9e3b6" d="M3710 1v1456c0 165 110 579 610 579h1280z"${3}/></g>`;
function tmpl$z($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$z(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_eps = registerTemplate(tmpl$z);
tmpl$z.stylesheets = [];
tmpl$z.stylesheetToken = "lwc-7fa5rn98g45";
tmpl$z.legacyStylesheetToken = "buildTemplates-doctype_eps";
freezeTemplate(tmpl$z);

const $fragment1$y = parseSVGFragment`<g${3}><path fill="#2e844a" d="M511 1A507 507 0 004 508v5384c0 280 227 508 507 508h4578c280 0 507-228 507-508V2032L3706 1z"${3}/><path fill="#fff" d="M1943 5394c-22 0-42-9-54-27l-373-497-374 497a64 64 0 01-54 27c-40 0-72-31-72-72 0-15 3-31 14-43l389-513-362-483c-9-13-14-27-14-42a72 72 0 01129-43l344 463 344-464c13-18 32-27 54-27 38 0 74 30 74 72 0 14-4 28-13 41l-362 481 389 515c9 12 13 27 13 41 0 40-33 74-72 74zm999-13h-546c-59 0-108-48-108-108V4241c0-39 33-71 78-71 39 0 72 32 72 72v1006h504c36 0 65 29 65 65 0 39-29 68-65 68zm698 22c-178 0-319-60-421-150a71 71 0 01-24-54c0-36 27-75 70-75 15 0 31 3 44 14 83 74 198 131 336 131 215 0 283-115 283-207 0-309-711-138-711-567 0-198 176-333 412-333 155 0 288 47 386 128 16 14 25 34 25 54 0 36-31 72-70 72-15 0-31-6-44-16a483 483 0 00-307-105c-144 0-247 78-247 191 0 270 711 115 711 564 0 175-119 353-443 353z"${3}/><path fill="#194e31" d="M5595 2035v100H4315s-631-126-612-671c0 1 20 571 600 571z"${3}/><path fill="#cdefc4" d="M3705 0v1456c0 166 110 579 610 579h1280z"${3}/></g>`;
function tmpl$y($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$y(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_excel = registerTemplate(tmpl$y);
tmpl$y.stylesheets = [];
tmpl$y.stylesheetToken = "lwc-5kv16ivtc61";
tmpl$y.legacyStylesheetToken = "buildTemplates-doctype_excel";
freezeTemplate(tmpl$y);

const $fragment1$x = parseSVGFragment`<g${3}><path fill="#747474" d="M511 3A507 507 0 004 510v5384c0 280 227 507 507 507h4578c280 0 507-227 507-507V2034L3706 3H511z"${3}/><path fill="#5c5c5c" d="M5596 2038v100H4316s-631-126-613-671c0 0 21 571 601 571h1292z"${3}/><path fill="#c9c9c9" d="M3706 3v1456c0 165 110 579 610 579h1280L3706 2z"${3}/><path fill="#fff" d="M1745 5392h-624c-60 0-108-49-108-108v-985c0-59 48-108 108-108h624c37 0 65 29 65 68 0 36-28 65-65 65h-583v387h571c36 0 65 29 65 69 0 36-29 65-65 65h-571v414h583c37 0 65 28 65 64 0 40-28 69-65 69zm1210 12c-21 1-41-8-54-26l-372-497-375 497a64 64 0 01-54 27c-40 0-72-31-72-72 0-15 4-31 14-44l389-513-362-482c-9-13-14-27-14-42 0-34 29-72 72-72 22 0 43 11 58 29l344 463 343-465c13-18 33-27 54-27a74 74 0 0162 114l-362 480 389 515c9 13 12 27 12 42 0 39-32 73-72 73zm1078-12h-624c-60 0-108-49-108-108v-985c0-59 48-108 108-108h624c36 0 65 29 65 68 0 36-29 65-65 65h-583v387h571c36 0 64 29 64 69 1 36-28 65-64 65h-571v414h583c36 0 65 28 65 64 0 40-29 69-65 69z"${3}/></g>`;
function tmpl$x($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$x(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_exe = registerTemplate(tmpl$x);
tmpl$x.stylesheets = [];
tmpl$x.stylesheetToken = "lwc-763srifhls9";
tmpl$x.legacyStylesheetToken = "buildTemplates-doctype_exe";
freezeTemplate(tmpl$x);

const $fragment1$w = parseSVGFragment`<g${3}><path fill="#ea001e" d="M511 1A507 507 0 004 508v5384c0 281 227 508 507 508h4578c280 0 507-227 507-508V2032L3706 1z"${3}/><path fill="#ba0517" d="M5596 2035v100H4316s-631-126-613-671c0 0 21 571 601 571z"${3}/><path fill="#fe8f7d" d="M3706-1v1457c0 165 110 579 610 579h1280z"${3}/><path fill="#fff" d="M1747 4311h-584v387h571c36 0 65 28 65 68 0 36-29 65-65 65h-571v484c0 42-32 74-75 74-42 0-74-32-74-74V4285c0-59 49-108 108-108h625c36 0 65 29 65 69-1 36-29 65-65 65zm955 1067h-545c-60 0-108-48-108-108V4238c0-39 32-71 77-71 40 0 72 32 72 72v1006h504c36 0 65 29 65 65 0 40-29 68-65 68zm1223 11c-31 0-60-18-72-48l-90-229h-598l-90 229a78 78 0 01-72 48 80 80 0 01-76-108l414-1033a130 130 0 01242 0l417 1033c4 9 6 20 6 29 0 36-33 79-81 79zm-461-1057l-256 647h510z"${3}/></g>`;
function tmpl$w($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$w(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_flash = registerTemplate(tmpl$w);
tmpl$w.stylesheets = [];
tmpl$w.stylesheetToken = "lwc-5p5a9ddu0t3";
tmpl$w.legacyStylesheetToken = "buildTemplates-doctype_flash";
freezeTemplate(tmpl$w);

const $fragment1$v = parseSVGFragment`<g${3}><path fill="#032d60" d="M5148 1546a452 452 0 01452 454v3100a452 452 0 01-452 452H448A452 452 0 010 5100V1300a452 452 0 01452-448h1148c382 0 423 114 674 445 207 274 711 249 1039 249z"${3}/><path fill="#90d0fe" d="M5148 1812H924a452 452 0 00-452 452v284l447-648h4235l446 649v-285a452 452 0 00-452-452z"${3}/><path fill="#1ab9ff" d="M457 5548h4691a452 452 0 00452-448V2348a452 452 0 00-452-448H925a452 452 0 00-452 452v2748a465 465 0 01-6 72 233 233 0 01-460 0 450 450 0 00450 376z"${3}/></g>`;
function tmpl$v($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$v(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_folder = registerTemplate(tmpl$v);
tmpl$v.stylesheets = [];
tmpl$v.stylesheetToken = "lwc-4mhi5rfm03u";
tmpl$v.legacyStylesheetToken = "buildTemplates-doctype_folder";
freezeTemplate(tmpl$v);

const $fragment1$u = parseSVGFragment`<g${3}><path fill="#1b96ff" d="M511 1A507 507 0 004 509v5384c0 280 227 507 507 507h4578c280 0 507-227 507-507V2032L3706 1H511z"${3}/><path fill="#fff" d="M1013 3744h2157v206H1013zm0 480h2157v206H1013zm0 480h2157v206H1013zm0 480h1224v206H1013z"${3}/><path fill="#0b5cab" d="M5596 2038v100H4316s-631-126-613-671c0 0 21 571 601 571h1292z"${3}/><path fill="#aacbff" d="M3706 3v1456c0 165 110 579 610 579h1280L3706 2z"${3}/></g>`;
function tmpl$u($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$u(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_gdoc = registerTemplate(tmpl$u);
tmpl$u.stylesheets = [];
tmpl$u.stylesheetToken = "lwc-1qner25aoc2";
tmpl$u.legacyStylesheetToken = "buildTemplates-doctype_gdoc";
freezeTemplate(tmpl$u);

const $fragment1$t = parseSVGFragment`<g${3}><path fill="#0176d3" d="M51 0A51 51 0 000 51v538c0 28 23 51 51 51h458c28 0 51-23 51-51V203L371 0z"${3}/><path fill="#014486" d="M560 204v10H432s-64-13-62-67c0 0 2 57 60 57z"${3}/><path fill="#aacbff" d="M371 0v146c0 16 11 58 61 58h128z"${3}/><path fill="#fff" d="M259 451h-76v22h54c-3 32-29 45-54 45a59 59 0 01-59-60c0-34 26-60 59-60 25 0 41 16 41 16l16-16s-20-22-57-22a84 84 0 00-83 82c0 42 34 83 84 83 44 0 77-30 77-75z"${3}/></g>`;
function tmpl$t($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 560 640",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$t(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_gdocs = registerTemplate(tmpl$t);
tmpl$t.stylesheets = [];
tmpl$t.stylesheetToken = "lwc-4q9930ae3bk";
tmpl$t.legacyStylesheetToken = "buildTemplates-doctype_gdocs";
freezeTemplate(tmpl$t);

const $fragment1$s = parseSVGFragment`<g${3}><path fill="#5a1ba9" d="M51 0A51 51 0 000 51v538c0 28 23 51 51 51h458c28 0 51-23 51-51V203L371 0z"${3}/><path fill="#300b60" d="M560 204v10H432s-63-13-61-68c0 0 2 57 60 57h129z"${3}/><path fill="#ad7bee" d="M371 0v146c0 16 11 58 61 58h128z"${3}/><path fill="#fff" d="M127 394c0-4-3-7-7-7h-13c-4 0-7 3-7 7v13c0 4 3 7 7 7h13c4 0 7-3 7-7zm188 0c0-4-3-7-7-7H151c-4 0-7 3-7 7v13c0 4 3 7 7 7h157c4 0 7-3 7-7zm-188 63c0-4-3-7-7-7h-13c-4 0-7 3-7 7v13c0 4 3 7 7 7h13c4 0 7-3 7-7zm188 0c0-4-4-7-8-7H152c-4 0-8 3-8 7v13c0 4 4 7 8 7h155c4 0 8-3 8-7zm-188 62c0-4-3-7-7-7h-13c-4 0-7 3-7 7v13c0 4 3 7 7 7h13c4 0 7-3 7-7zm188 0c0-4-3-7-7-7H151c-4 0-7 3-7 7v13c0 4 3 7 7 7h157c4 0 7-3 7-7z"${3}/></g>`;
function tmpl$s($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 560 640",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$s(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_gform = registerTemplate(tmpl$s);
tmpl$s.stylesheets = [];
tmpl$s.stylesheetToken = "lwc-2u7m527g61t";
tmpl$s.legacyStylesheetToken = "buildTemplates-doctype_gform";
freezeTemplate(tmpl$s);

const $fragment1$r = parseSVGFragment`<g${3}><path fill="#fcc003" d="M511 1A507 507 0 004 508v5384c0 281 227 508 507 508h4578c280 0 507-227 507-508V2032L3706 1z"${3}/><path fill="#2e2204" d="M1012 3747v1190h1242V3747zm1129 964H1125v-739h1016zm68-513v226h838v738H2031v-284h-113v510h1242V4198z"${3}/><path fill="#e4a201" d="M5596 2038v100H4316s-631-126-613-671c0 0 21 571 601 571z"${3}/><path fill="#f9e3b6" d="M3706 3v1456c0 165 110 579 610 579h1280L3706 2z"${3}/></g>`;
function tmpl$r($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$r(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_gpres = registerTemplate(tmpl$r);
tmpl$r.stylesheets = [];
tmpl$r.stylesheetToken = "lwc-5kn1orgdftn";
tmpl$r.legacyStylesheetToken = "buildTemplates-doctype_gpres";
freezeTemplate(tmpl$r);

const $fragment1$q = parseSVGFragment`<g${3}><path fill="#3ba755" d="M513 0A507 507 0 006 507v5384c0 281 227 508 507 508h4578c280 0 507-227 507-508V2031L3708 0z"${3}/><path fill="#fff" d="M1011 3747v1642h2152V3747zm646 1507h-497v-358h496zm0-501h-497v-358h496zm0-501h-497v-358h496zm1363 1002H1804v-358h1216zm0-501H1804v-358h1216zm0-501H1804v-358h1216z"${3}/><path fill="#22683e" d="M5598 2032v100H4318s-631-126-613-671c0 0 21 571 600 571z"${3}/><path fill="#cdefc4" d="M3708-3v1456c0 166 110 579 610 579h1280z"${3}/></g>`;
function tmpl$q($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$q(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_gsheet = registerTemplate(tmpl$q);
tmpl$q.stylesheets = [];
tmpl$q.stylesheetToken = "lwc-7ko5877jbau";
tmpl$q.legacyStylesheetToken = "buildTemplates-doctype_gsheet";
freezeTemplate(tmpl$q);

const $fragment1$p = parseSVGFragment`<g${3}><path fill="#ff5d2d" d="M514 1A507 507 0 006 508v5384c0 280 227 508 508 508h4577c280 0 507-228 507-508V2032L3708 1H513z"${3}/><path fill="#d83a00" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571z"${3}/><path fill="#feb9a5" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/><path fill="#fff" d="M1894 5084c-12 0-23-2-33-6l-758-338a138 138 0 010-250l758-340a89 89 0 01122 84c-1 34-19 67-53 82l-703 300 703 298c34 15 52 46 52 82 0 55-46 88-88 88zm847-1104l-443 1329a93 93 0 01-91 63c-52 0-92-40-92-88 0-11 2-19 6-30l443-1329c13-38 49-63 91-63 52 0 90 42 90 90zm1018 760l-758 338a89 89 0 01-69-164l703-298-703-300a88 88 0 01-55-82c-2-55 44-90 90-90 11 0 24 2 34 6l758 340a138 138 0 010 250z"${3}/></g>`;
function tmpl$p($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$p(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_html = registerTemplate(tmpl$p);
tmpl$p.stylesheets = [];
tmpl$p.stylesheetToken = "lwc-2d4j6pjf17d";
tmpl$p.legacyStylesheetToken = "buildTemplates-doctype_html";
freezeTemplate(tmpl$p);

const $fragment1$o = parseSVGFragment`<g${3}><path fill="#06a59a" d="M513 4A507 507 0 005 512v5384c0 280 227 507 508 507h4577c280 0 507-227 507-507V2035L3707 4H512z"${3}/><path fill="#056764" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571z"${3}/><path fill="#acf3e4" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/><path fill="#fff" d="M1012 5374V3283h2091v2091zm1880-1884H1223v1260h1669zm-959 838l391-526 121 213 140-44 98 563H1375l349-332zm-385-364c-91 0-165-69-165-154s74-154 165-154 165 69 165 154-74 154-165 154z"${3}/></g>`;
function tmpl$o($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$o(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_image = registerTemplate(tmpl$o);
tmpl$o.stylesheets = [];
tmpl$o.stylesheetToken = "lwc-eg73mib177";
tmpl$o.legacyStylesheetToken = "buildTemplates-doctype_image";
freezeTemplate(tmpl$o);

const $fragment1$n = parseSVGFragment`<g${3}><path fill="#f38303" d="M514 1A507 507 0 007 508v5384c0 280 227 507 507 507h4578c280 0 507-227 507-507V2031L3709 0H514z"${3}/><path fill="#a96404" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571h1293z"${3}/><path fill="#ffba90" d="M3707 0v1456c0 166 111 579 611 579h1280L3707 0z"${3}/><path fill="#371e03" d="M1782 5401c-20 0-40-9-54-25l-443-527-121 136v341c0 41-32 74-75 74-42 0-74-33-74-74V4249c0-40 32-72 77-72 40 0 72 32 72 72v560l519-607c14-16 32-23 52-23 40 0 74 30 74 70 0 15-5 31-16 41l-411 469 456 527c11 13 16 27 16 43 0 35-29 72-72 72zm1092-12h-624c-60 0-108-49-108-108v-985c0-60 48-108 108-108h624c37 0 65 28 65 68 0 36-28 65-64 65h-584v387h571c36 0 65 29 65 69 0 36-29 64-65 64h-571v415h583c37 0 65 28 65 64 0 40-28 69-64 69zm1239-1102l-403 594v445c0 41-33 73-76 73-41 0-74-32-74-73v-445l-405-594c-7-11-13-24-13-38 0-34 27-72 72-72 22 0 47 11 62 31l358 538 358-538c13-18 36-29 58-29 39 0 74 32 74 70 0 13-4 27-11 38z"${3}/></g>`;
function tmpl$n($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$n(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_keynote = registerTemplate(tmpl$n);
tmpl$n.stylesheets = [];
tmpl$n.stylesheetToken = "lwc-6bnaf3c2f4o";
tmpl$n.legacyStylesheetToken = "buildTemplates-doctype_keynote";
freezeTemplate(tmpl$n);

const $fragment1$m = parseSVGFragment`<g${3}><path fill="#032d60" d="M515 155c25 0 45 20 45 45v310c0 25-20 45-45 45H45a45 45 0 01-45-45V130c0-25 20-45 45-45h115c38 0 42 12 67 45 21 27 72 25 104 25h184z"${3}/><path fill="#90d0fe" d="M515 181H92a45 45 0 00-45 45v29l45-65h423l45 65v-29c0-25-20-45-45-45z"${3}/><path fill="#1ab9ff" d="M46 555h469c25 0 45-20 45-45V235c0-25-20-45-45-45H93a45 45 0 00-46 44v283c0 13-10 23-23 23a23 23 0 01-23-23 45 45 0 0045 38z"${3}/><path fill="#fff" d="M255 378h36v115h-36zm61 0h36v115h-36zm148-60l-150-62c-4-2-7-3-11-3-1 0-5 0-11 3l-150 62c-3 2-5 5-5 8v9c0 5 4 9 9 9h29v140c0 5 4 9 9 9h45V378h-18c-4 0-6-2-6-6v-12c0-4 2-6 6-6h183c4 0 6 2 6 6v12c0 4-2 6-6 6h-18v115h45c5 0 9-4 9-9V344h29c5 0 9-4 9-9v-8c1-4-1-7-4-9zm-161 5a19 19 0 110-38 19 19 0 010 38z"${3}/></g>`;
function tmpl$m($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 560 640",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$m(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_library_folder = registerTemplate(tmpl$m);
tmpl$m.stylesheets = [];
tmpl$m.stylesheetToken = "lwc-41lfslhq7od";
tmpl$m.legacyStylesheetToken = "buildTemplates-doctype_library_folder";
freezeTemplate(tmpl$m);

const $fragment1$l = parseSVGFragment`<g${3}><path fill="#1b96ff" d="M515 1A507 507 0 008 508v5384c0 280 227 507 507 507h4578c280 0 507-227 507-507V2032L3710 1z"${3}/><path fill="#0b5cab" d="M5598 2035v100H4318s-631-126-613-671c0 0 21 571 600 571z"${3}/><path fill="#aacbff" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/><path fill="#fff" d="M2825 3561c-207-210-545-210-755 0l-305 306c-207 207-207 545 0 755 48 48 125 48 174 0 50-48 50-126 0-173-112-114-112-294 0-408l308-306c112-112 294-112 406 0 112 117 118 294 0 406l-146 148c36 98 50 201 41 305l278-278c208-210 219-547 0-755zm-770 595c-48 48-48 128 0 176 114 112 114 294 0 406l-306 305c-112 112-294 112-406 0s-112-294 0-406l146-146c-37-98-50-203-41-308l-280 280c-208 208-208 545 0 755 209 207 547 207 754 0l308-308c208-208 208-545 0-755-48-48-128-48-175 0z"${3}/></g>`;
function tmpl$l($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$l(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_link = registerTemplate(tmpl$l);
tmpl$l.stylesheets = [];
tmpl$l.stylesheetToken = "lwc-2ss5qgdab4c";
tmpl$l.legacyStylesheetToken = "buildTemplates-doctype_link";
freezeTemplate(tmpl$l);

const $fragment1$k = parseSVGFragment`<g${3}><path fill="#730394" d="M512 1A507 507 0 004 508v5384c0 280 227 508 508 508h4577c280 0 508-228 508-508V2032L3706 1z"${3}/><path fill="#3d0157" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571z"${3}/><path fill="#e5b9fe" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/><path fill="#fff" d="M2113 5388c-41 0-74-33-74-74v-925l-403 976c-5 14-20 23-38 23-14 0-29-9-34-23l-402-976v925c0 42-32 74-75 74-42 0-74-32-74-74V4288c0-65 56-121 124-121 49 0 99 29 119 76l344 837 346-837c20-47 68-76 119-76 68 0 124 56 124 121v1026c0 42-33 74-76 74zm884-488h-334l1 414c0 42-33 74-76 74-41 0-74-32-74-74V4284c0-59 49-108 108-108h375c241 0 376 166 376 362s-139 362-376 362zm-18-591h-316l1 458h315c142 0 239-94 239-229s-97-229-239-229zm1391 764h-117v241c0 42-32 74-76 74-41 0-73-32-73-74v-241h-508c-38 0-74-33-74-81 0-16 5-34 15-49l480-722c24-34 65-56 108-56 69 0 128 60 128 126v650h117c34 0 63 29 63 67 0 36-29 65-63 65zm-266-758l-422 626h422z"${3}/></g>`;
function tmpl$k($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$k(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_mp4 = registerTemplate(tmpl$k);
tmpl$k.stylesheets = [];
tmpl$k.stylesheetToken = "lwc-5cid3vgg3o5";
tmpl$k.legacyStylesheetToken = "buildTemplates-doctype_mp4";
freezeTemplate(tmpl$k);

const $fragment1$j = parseSVGFragment`<g${3}><path fill="#730394" d="M515 1A507 507 0 008 508v5384c0 281 227 508 507 508h4578c280 0 507-227 507-508V2032L3710 1z"${3}/><path fill="#3d0157" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571z"${3}/><path fill="#e5b9fe" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/><path fill="#fff" d="M1012 3452v1408h1470V3452zm535 534v1408h1469V3986z"${3}/><path fill="#e5b9fe" d="M1547 3986h935v873l-935 1z"${3}/></g>`;
function tmpl$j($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$j(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_overlay = registerTemplate(tmpl$j);
tmpl$j.stylesheets = [];
tmpl$j.stylesheetToken = "lwc-6p2i3h2n27t";
tmpl$j.legacyStylesheetToken = "buildTemplates-doctype_overlay";
freezeTemplate(tmpl$j);

const $fragment1$i = parseSVGFragment`<g${3}><path fill="#0b5cab" d="M515 1A507 507 0 008 509v5384c0 280 227 507 507 507h4578c280 0 507-227 507-507V2032L3710 1z"${3}/><path fill="#032d60" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571z"${3}/><path fill="#aacbff" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/><path fill="#fff" d="M2919 4087v1192c0 66-53 119-119 119H1131c-66 0-119-53-119-119V4087c0-13 2-26 7-37l159-477c15-49 61-82 112-82h1351c51 0 97 33 112 82l159 477c5 11 7 24 7 37zm-245 0l-119-357H1376l-119 357z"${3}/></g>`;
function tmpl$i($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$i(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_pack = registerTemplate(tmpl$i);
tmpl$i.stylesheets = [];
tmpl$i.stylesheetToken = "lwc-1ju14nq9ivg";
tmpl$i.legacyStylesheetToken = "buildTemplates-doctype_pack";
freezeTemplate(tmpl$i);

const $fragment1$h = parseSVGFragment`<g${3}><path fill="#5867e8" d="M511-1A507 507 0 004 507v5384c0 280 227 507 507 507h4578c280 0 507-227 507-507V2030L3706-1z"${3}/><path fill="#2f2cb7" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571z"${3}/><path fill="#bec7f6" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/><path fill="#fff" d="M1496 4901h-333v414c0 41-33 74-76 74-41 0-74-33-74-74V4285c0-60 49-108 108-108h375c241 0 376 165 376 362s-139 362-376 362zm-18-591h-315v457h315c142 0 239-93 239-228s-97-229-239-229zm1206 1091a602 602 0 01-618-623c0-372 276-621 618-621 180 0 317 69 412 164 15 13 20 32 20 50 0 40-29 71-68 71-18 0-35-8-49-22a441 441 0 00-315-130c-263 0-463 202-463 488 0 285 200 490 463 490 144 0 263-70 322-130v-245h-358c-36 0-65-28-65-68 0-36 29-65 65-65h400c59 0 108 49 108 108v238c0 185-260 295-472 295zm1148-2c-178 0-319-59-421-149a71 71 0 01-24-54c0-36 27-76 71-76 14 0 30 4 43 15 83 74 198 131 337 131 214 0 282-115 282-207 0-310-711-139-711-567 0-198 176-333 412-333 155 0 288 47 386 128 16 14 25 34 25 54 0 36-31 72-70 72-15 0-31-6-44-16a482 482 0 00-308-105c-144 0-246 77-246 191 0 270 711 115 711 563 0 175-119 353-443 353z"${3}/></g>`;
function tmpl$h($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$h(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_pages = registerTemplate(tmpl$h);
tmpl$h.stylesheets = [];
tmpl$h.stylesheetToken = "lwc-2hpau7hfbgs";
tmpl$h.legacyStylesheetToken = "buildTemplates-doctype_pages";
freezeTemplate(tmpl$h);

const $fragment1$g = parseSVGFragment`<g${3}><path fill="#8e030f" d="M51 0A51 51 0 000 51v538c0 28 23 51 51 51h458c28 0 51-23 51-51V203L371 0z"${3}/><path fill="#640103" d="M560 204v10H432s-63-13-61-67c0 0 2 57 60 57z"${3}/><path fill="#feb8ab" d="M371 0v146c0 17 11 58 61 58h128z"${3}/><path fill="#fff" d="M149 490h-33v41c0 4-3 7-8 7-4 0-7-3-7-7V429c0-6 5-11 11-11h37c24 0 38 17 38 36 0 20-14 36-38 36zm-1-59h-32v46h32c14 0 24-9 24-23s-10-23-24-23zm104 107h-30c-6 0-11-5-11-11v-98c0-6 5-11 11-11h30c37 0 62 26 62 60s-24 60-62 60zm0-107h-26v93h26c29 0 46-21 46-47 1-25-16-46-46-46zm163 0h-58v39h57c4 0 6 3 6 7s-3 6-6 6h-57v48c0 4-3 7-8 7-4 0-7-3-7-7V429c0-6 5-11 11-11h62c4 0 6 3 6 7 1 3-2 6-6 6z"${3}/></g>`;
function tmpl$g($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 560 640",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$g(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_pdf = registerTemplate(tmpl$g);
tmpl$g.stylesheets = [];
tmpl$g.stylesheetToken = "lwc-1e1gl1p8s79";
tmpl$g.legacyStylesheetToken = "buildTemplates-doctype_pdf";
freezeTemplate(tmpl$g);

const $fragment1$f = parseSVGFragment`<g${3}><path fill="#ff5d2d" d="M511 0A507 507 0 004 507v5384c0 280 227 508 507 508h4578c280 0 507-228 507-508V2031L3706 0z"${3}/><path fill="#d83a00" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571z"${3}/><path fill="#feb9a5" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/><path fill="#281202" d="M1496 4901h-333v414c0 42-32 74-75 74-42 0-74-32-74-74V4285c0-59 49-108 108-108h374c242 0 377 166 377 362s-139 362-377 362zm-18-590h-315v457h315c143 0 240-94 240-229s-97-228-240-228zm1120 590h-333v414c0 42-33 74-76 74-41 0-74-32-74-74V4285c0-59 49-108 108-108h375c241 0 376 166 376 362s-138 362-376 362zm-18-590h-315v457h315c142 0 240-94 240-229-1-135-98-228-240-228zm1411 0h-328l1 1004c0 42-33 74-76 74-41 0-74-32-74-74V4310l-326 1c-36 0-65-29-65-69 1-36 29-65 65-65h803c36 0 65 29 65 69 0 36-29 65-65 65z"${3}/></g>`;
function tmpl$f($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$f(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_ppt = registerTemplate(tmpl$f);
tmpl$f.stylesheets = [];
tmpl$f.stylesheetToken = "lwc-4ve5f9gd1k9";
tmpl$f.legacyStylesheetToken = "buildTemplates-doctype_ppt";
freezeTemplate(tmpl$f);

const $fragment1$e = parseSVGFragment`<g${3}><path fill="#0176d3" d="M511 5A507 507 0 004 513v5384c0 280 227 507 507 507h4578c280 0 507-227 507-507V2036L3706 5z"${3}/><path fill="#014486" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571z"${3}/><path fill="#aacbff" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/><path fill="#fff" d="M1495 4901h-333v414c0 42-32 74-76 74-41 0-73-32-73-74V4285c0-59 48-108 108-108h374c242 0 377 166 377 362s-139 362-377 362zm-18-590h-315v457h315c142 0 240-94 240-229s-98-228-240-228zm1026 1089c-178 0-319-60-421-150a71 71 0 01-24-54c0-36 28-75 71-75 14 0 30 3 43 14 83 74 198 132 337 132 214 0 282-116 282-208 0-309-711-138-711-567 0-198 176-333 412-333 155 0 289 47 386 128 16 14 25 34 25 54 0 36-31 72-70 72-15 0-31-5-44-16a483 483 0 00-307-105c-144 0-247 78-247 191 0 270 711 115 711 564 0 175-119 353-443 353zm1122-22h-303c-59 0-108-48-108-108v-985c0-59 49-108 108-108h303c372 0 615 261 615 602 0 342-243 599-615 599zm0-1068h-261v935h261c291 0 461-209 461-466 0-260-164-469-461-469z"${3}/></g>`;
function tmpl$e($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$e(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_psd = registerTemplate(tmpl$e);
tmpl$e.stylesheets = [];
tmpl$e.stylesheetToken = "lwc-5hdtfb0b9n7";
tmpl$e.legacyStylesheetToken = "buildTemplates-doctype_psd";
freezeTemplate(tmpl$e);

const $fragment1$d = parseSVGFragment`<g${3}><path fill="#0176d3" d="M0 51v537c0 29 23 52 51 52h457c28 0 51-23 51-51V210L370 0H51A51 51 0 000 51z"${3}/><path fill="#aacbff" d="M370 0l190 210H421a52 52 0 01-52-52l1-158z"${3}/><path fill="#014486" d="M370 159v10c0 28 23 52 52 52h138v-10H421a53 53 0 01-51-52z"${3}/><path fill="#fff" d="M235 467h-69c-4 0-8 3-8 8s3 8 8 8h69c4 0 8-3 8-8s-4-8-8-8zm-70-54h69c4 0 8-3 8-8 0-4-3-8-8-8h-69c-4 0-8 3-8 8 1 5 4 8 8 8zm-23 19c-4 0-8 3-8 8 0 4 3 8 8 8h115c4 0 8-3 8-8 0-4-3-8-8-8H142zm126-65a100 100 0 00-139 2 100 100 0 00-2 139c19 20 45 32 73 32h92c4 0 8-3 8-8v-92c0-28-11-54-32-73zm16 158h-84a86 86 0 01-62-27 84 84 0 0156-142h5c47 0 85 38 85 85v84z"${3}/></g>`;
function tmpl$d($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 560 640",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$d(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_quip_doc = registerTemplate(tmpl$d);
tmpl$d.stylesheets = [];
tmpl$d.stylesheetToken = "lwc-3ljnam9du40";
tmpl$d.legacyStylesheetToken = "buildTemplates-doctype_quip_doc";
freezeTemplate(tmpl$d);

const $fragment1$c = parseSVGFragment`<g${3}><path fill="#3ba755" d="M0 51v537c0 29 23 52 51 52h457c28 0 51-23 51-51V210L370 0H51A51 51 0 000 51z"${3}/><path fill="#cdefc4" d="M370 0l190 210H421a52 52 0 01-52-52l1-158z"${3}/><path fill="#22683e" d="M370 159v10c0 28 23 52 52 52h138v-10H421a53 53 0 01-51-52z"${3}/><path fill="#fff" d="M235 467h-69c-4 0-8 3-8 8s3 8 8 8h69c4 0 8-3 8-8s-4-8-8-8zm-70-54h69c4 0 8-3 8-8 0-4-3-8-8-8h-69c-4 0-8 3-8 8 1 5 4 8 8 8zm-23 19c-4 0-8 3-8 8 0 4 3 8 8 8h115c4 0 8-3 8-8 0-4-3-8-8-8H142zm126-65a100 100 0 00-139 2 100 100 0 00-2 139c19 20 45 32 73 32h92c4 0 8-3 8-8v-92c0-28-11-54-32-73zm-68 158a86 86 0 01-62-27 84 84 0 0156-142h5c47 0 85 38 85 85v84h-84z"${3}/></g>`;
function tmpl$c($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 560 640",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$c(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_quip_sheet = registerTemplate(tmpl$c);
tmpl$c.stylesheets = [];
tmpl$c.stylesheetToken = "lwc-191snrnll70";
tmpl$c.legacyStylesheetToken = "buildTemplates-doctype_quip_sheet";
freezeTemplate(tmpl$c);

const $fragment1$b = parseSVGFragment`<g${3}><path fill="#9050e9" d="M0 51v537c0 29 23 52 51 52h457c28 0 51-23 51-51V210L370 0H51A51 51 0 000 51z"${3}/><path fill="#ece1f9" d="M370 0l190 210H421a52 52 0 01-52-52l1-158z"${3}/><path fill="#5a1ba9" d="M370 159v10c0 28 23 52 52 52h138v-10H421a53 53 0 01-51-52z"${3}/><path fill="#fff" d="M235 467h-69c-4 0-8 3-8 8s3 8 8 8h69c4 0 8-3 8-8s-4-8-8-8zm-70-54h69c4 0 8-3 8-8 0-4-3-8-8-8h-69c-4 0-8 3-8 8 1 5 4 8 8 8zm-23 19c-4 0-8 3-8 8 0 4 3 8 8 8h115c4 0 8-3 8-8 0-4-3-8-8-8H142zm126-65a100 100 0 00-139 2 100 100 0 00-2 139c19 20 45 32 73 32h92c4 0 8-3 8-8v-92c0-28-11-54-32-73zm-68 158a86 86 0 01-62-27 84 84 0 0156-142h5c47 0 85 38 85 85v84h-84z"${3}/></g>`;
function tmpl$b($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 560 640",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$b(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_quip_slide = registerTemplate(tmpl$b);
tmpl$b.stylesheets = [];
tmpl$b.stylesheetToken = "lwc-52tp532m9vj";
tmpl$b.legacyStylesheetToken = "buildTemplates-doctype_quip_slide";
freezeTemplate(tmpl$b);

const $fragment1$a = parseSVGFragment`<g${3}><path fill="#1b96ff" d="M511 1A507 507 0 004 508v5384c0 280 227 508 507 508h4578c280 0 507-228 507-508V2032L3706 1z"${3}/><path fill="#0b5cab" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571z"${3}/><path fill="#aacbff" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/><path fill="#fff" d="M1561 4883l258 390a73 73 0 01-63 110c-24 0-45-11-60-32l-293-456h-240v414c0 42-32 74-75 74-42 0-74-32-74-74V4279c0-59 48-108 108-108h374c220 0 377 141 377 362 0 216-148 335-312 350zm-398-578v459h315c142 0 240-96 240-231s-97-229-240-229h-315zm1747 0h-328v1004c0 42-33 74-76 74-41 0-74-32-74-74V4304l-325 1c-36 0-65-29-65-69 0-36 29-65 65-65h803c36 0 64 29 64 69 0 36-28 65-64 65zm1029 0h-583v387h570c37 0 65 28 65 68 0 36-28 65-64 65h-571v484c0 42-33 74-76 74-41 0-74-32-74-74V4279c0-59 49-108 108-108h625c36 0 65 29 65 69 0 36-29 65-65 65z"${3}/></g>`;
function tmpl$a($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$a(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_rtf = registerTemplate(tmpl$a);
tmpl$a.stylesheets = [];
tmpl$a.stylesheetToken = "lwc-118u4a21ghn";
tmpl$a.legacyStylesheetToken = "buildTemplates-doctype_rtf";
freezeTemplate(tmpl$a);

const $fragment1$9 = parseSVGFragment`<g${3}><path fill="#0d9dda" d="M515 1A507 507 0 008 508v5384c0 281 227 508 507 508h4578c280 0 507-227 507-508V2032L3710 1z"${3}/><path fill="#fff" d="M1012 3477v1909h1992V3477zm1811 1547H1193V3840h1630z"${3}/><path fill="#05628a" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571z"${3}/><path fill="#90d0fe" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/></g>`;
function tmpl$9($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$9(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_slide = registerTemplate(tmpl$9);
tmpl$9.stylesheets = [];
tmpl$9.stylesheetToken = "lwc-3cvjjp8jqdm";
tmpl$9.legacyStylesheetToken = "buildTemplates-doctype_slide";
freezeTemplate(tmpl$9);

const $fragment1$8 = parseSVGFragment`<g${3}><path fill="#fcc003" d="M507 1A507 507 0 000 508v5384c0 280 227 508 507 508h4578c280 0 507-228 507-508V2032L3702 1z"${3}/><path fill="#e4a201" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571z"${3}/><path fill="#f9e3b6" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/><path fill="#2e2204" d="M2940 3480H1177c-87 0-158 71-158 158v1599c0 87 71 158 158 158h1317l604-602V3638c0-87-71-158-158-158z"${3}/><path fill="#fcc003" d="M1374 4355h1368v165H1374zm0-355h1368v164H1374zm0 711h887v164h-887z"${3}/></g>`;
function tmpl$8($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$8(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_stypi = registerTemplate(tmpl$8);
tmpl$8.stylesheets = [];
tmpl$8.stylesheetToken = "lwc-1bcnjab5qas";
tmpl$8.legacyStylesheetToken = "buildTemplates-doctype_stypi";
freezeTemplate(tmpl$8);

const $fragment1$7 = parseSVGFragment`<g${3}><path fill="#fcc003" d="M515-4A507 507 0 008 504v5384c0 280 227 507 507 507h4578c280 0 507-227 507-507V2027L3710-4z"${3}/><path fill="#e4a201" d="M5601 2032v100H4321s-631-126-613-671c0 0 21 571 600 571z"${3}/><path fill="#f9e3b6" d="M3711-4v1457c0 165 110 579 610 579h1280z"${3}/><path fill="#2e2204" d="M1876 4305h-327v1004c0 42-33 74-76 74-41 0-74-32-74-74V4304l-326 1c-36 0-64-29-64-69 0-36 28-65 64-65h803c36 0 65 29 65 69 0 36-29 65-65 65zm1170 1080c-21 0-41-9-54-27l-372-497-375 497a64 64 0 01-54 27c-40 0-72-31-72-72 0-15 4-31 14-43l389-513-362-483c-9-13-14-27-14-42 0-34 29-72 72-72 22 0 43 11 58 29l344 463 343-465c13-18 33-27 55-27a74 74 0 0161 114l-362 481 389 514c9 13 12 27 12 42 0 40-32 74-72 74zm1120-1080h-328v1004c0 42-32 74-75 74-42 0-74-32-74-74V4304l-326 1c-36 0-65-29-65-69 0-36 29-65 65-65h803c36 0 65 29 65 69 0 36-29 65-65 65z"${3}/></g>`;
function tmpl$7($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$7(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_txt = registerTemplate(tmpl$7);
tmpl$7.stylesheets = [];
tmpl$7.stylesheetToken = "lwc-3mcbbdp4ni6";
tmpl$7.legacyStylesheetToken = "buildTemplates-doctype_txt";
freezeTemplate(tmpl$7);

const $fragment1$6 = parseSVGFragment`<g${3}><path fill="#747474" d="M511 1A507 507 0 004 508v5384c0 280 227 508 507 508h4578c280 0 507-228 507-508V2032L3706 1z"${3}/><path fill="#5c5c5c" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571z"${3}/><path fill="#c9c9c9" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/></g>`;
function tmpl$6($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$6(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_unknown = registerTemplate(tmpl$6);
tmpl$6.stylesheets = [];
tmpl$6.stylesheetToken = "lwc-4cb2dvjhcpd";
tmpl$6.legacyStylesheetToken = "buildTemplates-doctype_unknown";
freezeTemplate(tmpl$6);

const $fragment1$5 = parseSVGFragment`<g${3}><path fill="#730394" d="M515 1A507 507 0 008 509v5384c0 280 227 507 507 507h4578c280 0 507-227 507-507V2032L3710 1z"${3}/><path fill="#3d0157" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571z"${3}/><path fill="#e5b9fe" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/><path fill="#fff" d="M2453 4553c0 37-16 74-45 98-20 16-525 442-1184 710a135 135 0 01-119-8c-34-21-59-58-61-99a8228 8228 0 010-1401c2-40 27-77 62-100a130 130 0 01118-8 4826 4826 0 011184 712c29 22 45 59 45 96z"${3}/></g>`;
function tmpl$5($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$5(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_video = registerTemplate(tmpl$5);
tmpl$5.stylesheets = [];
tmpl$5.stylesheetToken = "lwc-4r4mds5aq6v";
tmpl$5.legacyStylesheetToken = "buildTemplates-doctype_video";
freezeTemplate(tmpl$5);

const $fragment1$4 = parseSVGFragment`<g${3}><path fill="#0b5cab" d="M511 1A507 507 0 004 508v5384c0 280 227 508 507 508h4578c280 0 507-228 507-508V2032L3706 1z"${3}/><path fill="#032d60" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571z"${3}/><path fill="#aacbff" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/><path fill="#fff" d="M2092 4269l-416 1033c-20 49-69 81-119 81h-4c-52 0-101-32-121-81l-414-1033c-3-9-5-18-5-29 0-36 32-79 81-79 31 0 59 18 72 48l389 999 389-999c11-29 39-48 72-48a80 80 0 0176 108zm314 1114c-41 0-74-32-74-74V4232c0-39 33-72 78-72 39 0 72 33 72 72v1077c0 42-33 74-76 74zm790 11c-178 0-318-60-421-150a71 71 0 01-23-54c0-36 27-75 70-75 14 0 30 3 43 14 83 74 198 132 337 132 214 0 282-116 282-208 0-309-711-138-711-567 0-198 177-333 413-333 154 0 288 47 385 128 16 14 25 34 25 54 0 36-31 72-70 72-15 0-31-5-43-16a483 483 0 00-308-105c-144 1-247 78-247 191 0 270 711 116 711 564 0 175-119 353-443 353z"${3}/></g>`;
function tmpl$4($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$4(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_visio = registerTemplate(tmpl$4);
tmpl$4.stylesheets = [];
tmpl$4.stylesheetToken = "lwc-3retb4mor4v";
tmpl$4.legacyStylesheetToken = "buildTemplates-doctype_visio";
freezeTemplate(tmpl$4);

const $fragment1$3 = parseSVGFragment`<g${3}><path fill="#3ba755" d="M515 1A507 507 0 008 509v5384c0 280 227 507 507 507h4578c280 0 507-227 507-507V2032L3710 1z"${3}/><path fill="#22683e" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571z"${3}/><path fill="#cdefc4" d="M3707 0v1456c0 166 111 579 611 579h1280zM2046 5385a978 978 0 000-1953z"${3}/><path fill="#fff" d="M1934 5385a978 978 0 010-1952z"${3}/></g>`;
function tmpl$3($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$3(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_webex = registerTemplate(tmpl$3);
tmpl$3.stylesheets = [];
tmpl$3.stylesheetToken = "lwc-2urdbpcgrkc";
tmpl$3.legacyStylesheetToken = "buildTemplates-doctype_webex";
freezeTemplate(tmpl$3);

const $fragment1$2 = parseSVGFragment`<g${3}><path fill="#107cad" d="M511 0A507 507 0 000 500v5388a507 507 0 00511 512h4578a507 507 0 00511-512V2028L3706 0z"${3}/><path fill="#084968" d="M5600 2035v100H4318s-631-126-613-671c0 0 21 571 600 571z"${3}/><path fill="#90d0fe" d="M3707 0v1456a578 578 0 00611 579h1282z"${3}/><path fill="#fff" d="M1424 5386h-300a108 108 0 01-108-108v-985a108 108 0 01108-108h300a600 600 0 110 1200zm0-1067h-261v934h261a441 441 0 00461-466 438 438 0 00-461-468zm1442 1089a586 586 0 01-600-621 600 600 0 111192 0 587 587 0 01-592 621zm0-1109c-270 0-441 207-441 488s171 488 441 488 441-209 441-488-172-487-441-487zm1845 38a75 75 0 0120 52 71 71 0 01-70 72 64 64 0 01-51-24 406 406 0 00-300-138 461 461 0 00-463 488 463 463 0 00463 488 400 400 0 00300-137 70 70 0 0151-24 72 72 0 0170 74 78 78 0 01-20 51 533 533 0 01-400 169 622 622 0 010-1243 526 526 0 01400 172z"${3}/></g>`;
function tmpl$2($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$2(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_word = registerTemplate(tmpl$2);
tmpl$2.stylesheets = [];
tmpl$2.stylesheetToken = "lwc-3sgiff3gicr";
tmpl$2.legacyStylesheetToken = "buildTemplates-doctype_word";
freezeTemplate(tmpl$2);

const $fragment1$1 = parseSVGFragment`<g${3}><path fill="#f38303" d="M511-2A507 507 0 004 505v5384c0 280 227 508 507 508h4578c280 0 507-228 507-508V2029L3706-2z"${3}/><path fill="#a96404" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571z"${3}/><path fill="#ffba90" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/><path fill="#371e03" d="M1937 5385c-22 0-41-9-54-27l-373-497-374 497a64 64 0 01-54 27c-40 0-72-31-72-72 0-15 3-31 14-43l389-513-362-483c-9-13-14-27-14-42 0-34 29-72 72-72 21 0 43 11 57 29l344 463 344-465c13-18 33-27 54-27 38 0 74 31 74 72 0 15-4 29-13 42l-362 481 389 514a74 74 0 01-59 115zm1446-2c-42 0-74-32-74-74v-925l-404 976c-5 14-20 23-37 23-15 0-29-9-35-23l-401-976v925c0 42-33 74-76 74-41 0-74-32-74-74V4283c0-65 56-121 125-121 48 0 99 29 118 76l344 837 346-837c20-47 68-76 119-76 68 0 124 56 124 121v1026c0 42-32 74-75 74zm1054-11h-545c-60 0-108-48-108-108V4232c0-39 32-72 77-72 40 0 72 33 72 72v1007h504c36 0 65 29 65 65 0 39-29 68-65 68z"${3}/></g>`;
function tmpl$1($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1$1(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_xml = registerTemplate(tmpl$1);
tmpl$1.stylesheets = [];
tmpl$1.stylesheetToken = "lwc-20b6g6r1f9m";
tmpl$1.legacyStylesheetToken = "buildTemplates-doctype_xml";
freezeTemplate(tmpl$1);

const $fragment1 = parseSVGFragment`<g${3}><path fill="#747474" d="M511-3A507 507 0 004 505v5384c0 280 227 507 507 507h4578c280 0 507-227 507-507V2028L3706-3z"${3}/><path fill="#5c5c5c" d="M5598 2035v100H4318s-631-126-613-671c0 1 21 571 600 571z"${3}/><path fill="#c9c9c9" d="M3707 0v1456c0 166 111 579 611 579h1280z"${3}/><path fill="#fff" d="M1844 5391h-758c-38 0-76-35-76-83 0-18 5-36 16-51l668-934h-621c-36 0-65-29-65-69 0-35 29-64 65-64h745c38 0 76 34 76 83 0 18-5 36-16 50l-668 934h634c36 0 64 29 64 65 1 40-28 69-64 69zm401 10c-41 0-74-32-74-73V4251c0-40 33-72 78-72 39 0 72 32 72 72v1077c0 41-33 73-76 73zm884-488l-333 1v414c0 41-33 73-76 73-41 0-74-32-74-73V4298c0-60 49-108 109-108h374c241 0 376 165 376 362s-138 362-376 362zm-18-590h-315v457h315c142 0 239-93 239-229 0-134-97-228-239-228z"${3}/></g>`;
function tmpl($api, $cmp, $slotset, $ctx) {
  const {st: api_static_fragment, h: api_element} = $api;
  return [api_element("svg", {
    className: $cmp.computedClass,
    attrs: {
      "focusable": "false",
      "data-key": $cmp.name,
      "aria-hidden": "true",
      "viewBox": "0 0 5600 6400",
      "part": "icon"
    },
    key: 0,
    svg: true
  }, [api_static_fragment($fragment1(), 2)])];
  /*LWC compiler v6.0.0*/
}
var doctype_zip = registerTemplate(tmpl);
tmpl.stylesheets = [];
tmpl.stylesheetToken = "lwc-42f7fk123rc";
tmpl.legacyStylesheetToken = "buildTemplates-doctype_zip";
freezeTemplate(tmpl);

var templates = {
  doctype_ai,
  doctype_attachment,
  doctype_audio,
  doctype_box_notes,
  doctype_csv,
  doctype_eps,
  doctype_excel,
  doctype_exe,
  doctype_flash,
  doctype_folder,
  doctype_gdoc,
  doctype_gdocs,
  doctype_gform,
  doctype_gpres,
  doctype_gsheet,
  doctype_html,
  doctype_image,
  doctype_keynote,
  doctype_library_folder,
  doctype_link,
  doctype_mp4,
  doctype_overlay,
  doctype_pack,
  doctype_pages,
  doctype_pdf,
  doctype_ppt,
  doctype_psd,
  doctype_quip_doc,
  doctype_quip_sheet,
  doctype_quip_slide,
  doctype_rtf,
  doctype_slide,
  doctype_stypi,
  doctype_txt,
  doctype_unknown,
  doctype_video,
  doctype_visio,
  doctype_webex,
  doctype_word,
  doctype_xml,
  doctype_zip
};

var iconSvgTemplatesDoctype = registerComponent(templates, {
  tmpl: _tmpl,
  sel: "lightning-icon-svg-templates-doctype",
  apiVersion: 61
});

export { iconSvgTemplatesDoctype as default };
